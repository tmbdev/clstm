...
double err(Sequence &a, Sequence &b) {
  ...
  for (int t=0; t<N; t++)
    for (int i=0; i<n; i++) 
      for (int j=0; j<m; j++)
        total += sqr(a[t](i,j) - b[t](i,j));
  return total;
}

void zero_grad(Network net) {
  net->params("", [](const string &s, Params *p) {
    p->zeroGrad();
  });
}
void get_params(vector<Params> &params, Network net) {
  params.clear();
  net->params("", [&params](const string &s, Params *p) {
...
int main(int argc, char **argv) {
  TRY {
    ...
    test_net(layer("Reversed", 7, 3, {},
                   {layer("SigmoidLayer", 7, 3, {}, {})}));
    test_net(layer("Parallel", 7, 3, {},
                   {layer("SigmoidLayer", 7, 3, {}, {}),
                   layer("LinearLayer", 7, 3, {}, {})}));
    test_net(make_net("bidi",
             {{"ninput", 7},
             {"noutput", 3},
             {"nhidden", 5},
