...
struct Tensor2 {
  ...
  // call.
  //
  // With these shorthands, you can write expressions like:
  //
  // x = x() * 2;
  //
  // This will multiply every element of x by 2.
  // The LHS `x` is a tensor map with the .device(...) applied;
  // the RHS `x()` is a tensor map without the .device(...) applied.
  //
  // For more complicated LHS expressions, use this form:
  //
  // x>> x.chip(1,0) = x.chip(1,0) * 2;
  //
  // The `x>>` "injects" the device from the tensor into the
  // LHS of the assignment. It will expand the expression into
  // multiple statements that cover all the possible devices handled
  // by tensors.
  //
  // Sorry this is so complicated; the reason is that Tensor evaluation
  // on different devices look syntactically the same but represents
  // really completely different code that gets generated by combinations
  // of overloading and template instantiations.
