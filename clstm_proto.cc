...
bool proto_verbose = getenv("clstm_proto_verbose") && \
    atoi(getenv("clstm_proto_verbose"));
...
void Mat_of_proto(Mat &a, const clstm::Array *array) {
    if (array->dim_size() != 2) throwf("bad format (Mat, %s, %d)",
                                       array->name().c_str(),
                                       array->dim_size());
...
Network net_of_proto(const clstm::NetworkProto *proto) {
    ...
    for (int i = 0; i < proto->weights_size(); i++) {
        ...
        if (a.mat) Mat_of_proto(*a.mat, &proto->weights(i));
        else if (a.vec) Vec_of_proto(*a.vec, &proto->weights(i));
    }
