...
void batches_of_image_0(Sequence &batches, mdarray<float> &image, int depth, int border=0) {
...
void image_of_batches_0(mdarray<float> &image, Sequence &batches, int depth, int border=0) {
    int d0 = batches.size();
...
struct Vstrips : public Image2Image {
    ...
    void forward() {
        ...
        // print("inputs", net->inputs.size(), ROWS(net->inputs[0]), COLS(net->inputs[0]), "/", net->ninput());
        net->forward();
...
int main3(int argc, char **argv) {
    ...
    try {
        ...
        for (int trial = 0; trial < 1000000; trial++) {
            ...
            net->forward();
            print("output", net->output.dim(0), net->output.dim(1), net->output.dim(2));
    ...
    try {
        ...
        for (int trial = 0; trial < 1000000; trial++) {
            ...
            if (trial%10000 == 0) {
                ...
                for (int t = 0; t < seq.size(); t++)
                    print(t, ":", seq[t](0, 0), seq[t](1, 0), " / ", out[t](0, 0), out[t](1, 0));
            }
        }
    } catch(const char *msg) {
        print("ERROR:", msg);
...
int main5(int argc, char **argv) {
    ...
    try {
        ...
        for (int trial = 0; trial < 1000000; trial++) {
            ...
            net->d_output -= output;
            if (trial%10000 == 0) {
                print("in", input.min(), input.max(),
                      "out", output.min(), output.max(),
                      "err", net->d_output.norm());
                for (int t = 0; t < input.dim(0); t++)
                    print(t, ":", input(t, 0, 0), input(t, 0, 1), " / ", output(t, 0, 0), output(t, 0, 1));
            }
            net->backward();
            net->update();
        }
